...
import com.layermark.survey.lib.dto.ForgetPasswordRenewalDTO;
import com.layermark.survey.lib.dto.ForgetPasswordRequestDTO;
...
public class AuthController {
    ...
    public void forgetPasswordEmailSend(@RequestBody ForgetPasswordRequestDTO forgetPasswordRequestDTO) {
        User user = userService.findByEmail(forgetPasswordRequestDTO.getEmail()); // Finding user with email.

        if (user != null) {
            try {
                String uuid = UUID.randomUUID().toString(); // Creating a random token.
                Timestamp tokenEndDate = new Timestamp(System.currentTimeMillis() + 3600000); // Creating token end date with
                // 60 minutes later from request date.

                // Sending email to user with created token information.
                emailSender.sendMail(forgetPasswordRequestDTO.getEmail(), "Survey - Forget Password Token",
    ...
    @PostMapping("/login/password/renewal")
    public void forgetPasswordRenewal(@RequestBody ForgetPasswordRenewalDTO forgetPasswordRenewalDTO) {
        User user = userService.findByEmail(forgetPasswordRenewalDTO.getEmail()); // Finding yser with email.
